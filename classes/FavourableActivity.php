...
// 	const LOCKED 	 = 2;//锁定
     ...
     * @return array
     */
    public static function FavourableInfo($act_id = 0) {
    	$favourable = [];
    	if ($act_id) {
    		...
    		$favourable = $db->where('act_id', $act_id)->first();
    		if (!empty ($favourable)) {
     ...
     * @param  $act_id
     */
    public static function FavourableRemove($act_id) {
    	if (!empty($act_id)) {
    		if (is_array($act_id)) {
    			RC_DB::table('favourable_activity')->whereIn('act_id', $act_id)->delete();
    		} else {
    			RC_DB::table('favourable_activity')->where('act_id', $act_id)->delete();
    		}
    	}
    }
     ...
     */
    public static function FavourableManage($parameter = array()) {

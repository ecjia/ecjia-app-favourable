...
// 		RC_Style::enqueue_style('datepicker',   RC_Uri::admin_url('statics/lib/datepicker/datepicker.css'));
	...
	public function insert() 
	{
        ...
        $content = '优惠活动方式：'.$act_type.'，优惠名：'.$act_name;
	...
	{ 
		...
		$links = array(
			array('href' => RC_Uri::url('favourable/admin/init'), 'text' => RC_Lang::lang('back_favourable_list'))
		);
	...
	public function remove() 
	{
        ...
        $content = '优惠活动方式：'.$act_type.'，优惠名：'.$name;
	...
	public function edit_act_name() 
	{
		...
		/* 对编辑优惠活动名称进行权限检查  BY：MaLiuWei  START */
		if (!empty($_SESSION['ru_id'])) {
			$this->showmessage(__('入驻商家没有操作权限，请登陆商家后台操作！'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
		}
		/* 对编辑优惠活动名称进行权限检查  BY：MaLiuWei  END */
	...
	public function edit_sort_order() 
	{
		...
		/* 对排序进行权限检查  BY：MaLiuWei  START */
		if (!empty($_SESSION['ru_id'])) {
			$this->showmessage(__('入驻商家没有操作权限，请登陆商家后台操作！'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
		}
		/* 对排序进行权限检查  BY：MaLiuWei  END */
		$id  = intval($_POST['pk']);
